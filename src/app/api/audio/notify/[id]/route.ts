...
export async function POST(
  ...
  for (const email of [...subs.map(s=>s.email), ...clients.map(c=>c.email)]) {
    ...
    const unsubLink = client ? `${process.env.APP_URL}/api/unsub/${client.unsubToken}` : `${process.env.APP_URL}`
    try {
      await sgMail.send({
        to: email,
      ...
      console.error("SENDGRID ERROR", e.code, e.response?.body)
      return new Response("Email failed", { status: 500 })
    }
  }

  await prisma.audio.update({ where: { id: audio.id }, data: { notifiedAt: new Date() } })
